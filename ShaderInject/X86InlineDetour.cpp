...
namespace {
    static_assert(sizeof(void*) == 4, "ShaderInject inline detours require an x86 build");
...
namespace x86hook {
    bool InlineDetour::Install(void* entryPoint, void* replacement,
        ...
        if (!entryPoint || !replacement || !originalStorage || IsInstalled()) {
            return false;
        }
        ...
        *reinterpret_cast<std::int32_t*>(trampoline + stolenSize + 1) =
        ...
        InterlockedExchangePointer(originalStorage, trampoline);
