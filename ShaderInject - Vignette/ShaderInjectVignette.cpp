...
int HandleException(EXCEPTION_POINTERS* exception) {
    char message[192] = {};
    sprintf_s(message, "[Vignette] Exception 0x%08lX at %p.",
...
void __cdecl OnFinalScene(const ShaderInjectFrameContext* frame) {
    ...
    __try {
        if (!VignetteEffect::Apply()) {
            if (!g_failureLogged) {
                g_failureLogged = true;
                Log("[Vignette] OpenGL shader resources are unavailable.");
            }
            return;
        }
        if (!g_firstFrameLogged) {
            g_firstFrameLogged = true;
            Log("[Vignette] First final-scene vignette pass completed.");
        }
    }
...
extern "C" BOOL __cdecl KPatch_Initialize() {
    ...
    if (!VignetteEffect::Initialize(shaderInject)) {
        InterlockedExchange(&initialized, 0);
        return FALSE;
    }
    ...
    const ShaderInjectProviderV1 provider = {
        ...
        nullptr,
        nullptr,
        SHADER_INJECT_PRIORITY_VIGNETTE,
    };
    if (!registerProvider || !registerProvider(&provider)) {
        InterlockedExchange(&initialized, 0);
        return FALSE;
    }
    Log("[Vignette] ShaderInject vignette provider loaded.");
    return TRUE;
}
